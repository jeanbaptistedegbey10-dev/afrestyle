  ...
  return (
    ...
    <section className="relative overflow-hidden py-20 text-center text-band-text">
      ...
      <div
        ...
        style={{ background: "radial-gradient(circle, var(--gold-soft) 0%, transparent 65%)" }}
      />
      ...
      <div className="relative z-10 container mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl">
        <p className="text-gold text-xs tracking-[0.25em] uppercase mb-4 flex items-center justify-center gap-3">
          <span className="h-px w-8 bg-gold/60 inline-block" />
          Le Cercle AfroStyle
          <span className="h-px w-8 bg-gold/60 inline-block" />
        </p>

        <h2 className="font-serif text-3xl md:text-5xl font-bold mb-4 text-band-text leading-tight">
          Rejoignez la <em className="text-gold">communauté</em>
        </h2>
        ...
        {status === "success" ? (
          <div className="inline-flex items-center gap-3 px-8 py-5 border border-gold/40 bg-white/5 rounded-sm">
            <span className="w-8 h-8 rounded-full bg-gold text-band flex items-center justify-center text-sm font-bold">
              ✓
            </span>
          ...
          <form
            ...
            className="flex max-w-md mx-auto bg-white/[0.06] backdrop-blur-sm border border-white/10 rounded-sm overflow-hidden transition-colors duration-300 focus-within:border-gold/60"
          >
            <input
              type="email"
              value={email}
            ...
            <button
              ...
              className="px-6 py-3.5 text-xs font-medium tracking-widest uppercase bg-gold text-band transition-all duration-300 hover:bg-gold-dark hover:shadow-[0_0_20px_var(--gold-soft)] disabled:opacity-60"
            >
